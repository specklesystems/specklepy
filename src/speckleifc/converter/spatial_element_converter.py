...
def spatial_element_to_speckle(
    ...
    direct_geometry = _convert_as_data_object(display_value, step_element, current_storey)
...
def _convert_as_data_object(
    display_value: list[Base], step_element: entity_instance, current_storey: str | None = None
) -> DataObject:
    guid = cast(str, step_element.GlobalId)
    name = cast(str, step_element.Name or step_element.LongName or guid)
