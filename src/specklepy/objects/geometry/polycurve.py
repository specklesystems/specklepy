...
class Polycurve(Base, IHasUnits, ICurve, IHasArea, speckle_type="Objects.Geometry.Polycurve"):
    """
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(segments: {len(self.segments)}, closed: {self.is_closed()}, units: {self.units})"
    ...
    def is_closed(self, tolerance: float = 1e-6) -> bool:
        ...
        start_pt = getattr(first_segment, "start")
        end_pt = getattr(last_segment, "end")
