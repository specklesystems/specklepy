...
@pytest.mark.run()
class TestProjectResourcePermissions:
    @pytest.fixture()
    ...
    def test_get_projects_with_permissions(
        self, client: SpeckleClient, test_project: Project
    ):  # noqa: E501
        """test getting projects with their permissions."""
        result = client.project.get_projects_with_permissions()
    ...
    def test_get_projects_with_permissions_with_filter(
        self, client: SpeckleClient, test_project: Project
    ):
        """test getting projects with permissions using a filter."""
        filter = UserProjectsFilter(search=test_project.name)

        result = client.project.get_projects_with_permissions(filter=filter)
    ...
    def test_get_projects_with_permissions_pagination(self, client: SpeckleClient):
        """test pagination parameters work correctly."""
        result = client.project.get_projects_with_permissions(limit=1)
        ...
        if result.total_count > 0:
            ...
            if result.total_count > 1:
                ...
                next_result = client.project.get_projects_with_permissions(
                    limit=1, cursor=result.cursor
                )
                assert isinstance(next_result, ResourceCollection)

    def test_projects_with_permissions_method_exists(self, client: SpeckleClient):
        """test that the method exists and is callable."""
        assert hasattr(client.project, "get_projects_with_permissions")
        method = client.project.get_projects_with_permissions
